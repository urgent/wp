(function ($) {
    ...
    $('.cmb-repeatable-group').on('cmb2_add_group_row_start', function (event, instance) {
        ...
        $oldRow.find('.tm_select2').each(function () {
    ...
    $('.cmb-repeatable-group').on('cmb2_add_row', function (event, newRow) {
        $(newRow).find('.tm_select').each(function () {
        ...
        $(newRow).find('.tm_multiselect').each(function () {
            $('option:selected', this).removeAttr("selected");
            $(this).select2_sortable();
        });

        // Reinitialise the field we previously destroyed
        $(newRow).prev().find('.tm_select').each(function () {
            $(this).select2({
                allowClear: true
            });
        });
        ...
        // Reinitialise the field we previously destroyed
        $(newRow).prev().find('.tm_multiselect').each(function () {
            $(this).select2_sortable();
        });
    });
    ...
    $('.cmb-repeatable-group').on('cmb2_shift_rows_start', function (event, instance) {
        ...
        groutmrap.find('.tm_select2').each(function () {
            $(this).select2('destroy');
        });
    ...
    $('.cmb-repeatable-group').on('cmb2_shift_rows_complete', function (event, instance) {
        var groutmrap = $(instance).closest('.cmb-repeatable-group');
        groutmrap.find('.tm_select').each(function () {
            $(this).select2({
                allowClear: true
            });
        });

        groutmrap.find('.tm_multiselect').each(function () {
            $(this).select2_sortable();
        });
    });
